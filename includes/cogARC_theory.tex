...
\todo{Choose this:}
We could have added the rotation to the cubes transforms, but we only needed to know the direction of collisions. We made an object that get its rotation directly from the gyroscope. To this object we added a child object that have a local position which will align with the rotation to its grandparent object. This gave us a unit vector for the direction up in the real world.

\todo{Or choose this:} \\
So we made an object that rotates as the rotation from the gyroscope and that contains a child object thats local position is translated one unit up from parent object. This gives us a unit vector for the direction up in the real world.
...
Then we can calculate the angle by a little bit of math:
